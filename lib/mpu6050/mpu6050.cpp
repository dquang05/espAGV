...
esp_err_t MPU6050::whoAmI(uint8_t *out)
{
  if (!out)
    return ESP_ERR_INVALID_ARG;
  return readReg_(REG_WHO_AM_I, out);
}
...
esp_err_t MPU6050::begin()
{
  ...
  // Defaults options
  if ((err = setDLPF(3)) != ESP_OK)
    return err;
  if ((err = setSampleRate(100)) != ESP_OK)
    return err;
  if ((err = setAccelRange(accel_fs_)) != ESP_OK)
    return err;
  if ((err = setGyroRange(gyro_fs_)) != ESP_OK)
    return err;
...
esp_err_t MPU6050::readScaledBody(mpu6050_scaled_t *out_body)
{
  if (!out_body)
    return ESP_ERR_INVALID_ARG;
  ...
  out_body->gy_dps = gy;
  out_body->gz_dps = gz;
  out_body->temp_c = s.temp_c;

  return ESP_OK;
}

esp_err_t MPU6050::computeAnglesFromScaled(const mpu6050_scaled_t &s_body, float *roll_deg, float *pitch_deg, float dt_s)
{
  if (dt_s <= 0.0f)
    return ESP_ERR_INVALID_ARG;
  if (dt_s > 1.0f)
  ...
  // accel angles
  float roll_acc = atan2f(s_body.ay_g, s_body.az_g) * RAD2DEG;
  float pitch_acc = atan2f(-s_body.ax_g, sqrtf(s_body.ay_g * s_body.ay_g + s_body.az_g * s_body.az_g)) * RAD2DEG;

  // complementary filter (gyro already in body axes)
  roll_deg_ = alpha_ * (roll_deg_ + s_body.gx_dps * dt_s) + (1.0f - alpha_) * roll_acc;
  pitch_deg_ = alpha_ * (pitch_deg_ + s_body.gy_dps * dt_s) + (1.0f - alpha_) * pitch_acc;
...
esp_err_t MPU6050::readScaledAndAngles(mpu6050_scaled_t *out_s_body, float *roll, float *pitch, float dt_s)
{
  if (!out_s_body)
    return ESP_ERR_INVALID_ARG;

  esp_err_t err = readScaledBody(out_s_body); // I2C chỉ 1 lần ở đây
  if (err != ESP_OK)
    return err;

  return computeAnglesFromScaled(*out_s_body, roll, pitch, dt_s);
}
...
{
  mpu6050_scaled_t s_body{};
  float r = 0, p = 0;
  esp_err_t err = readScaledAndAngles(&s_body, &r, &p, dt_s);
  if (err != ESP_OK)
    return err;
